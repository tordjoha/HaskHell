...
-- Constructs the background for the menu screen using a square texture
...
-- Constructs the game platform  with a repeating texture
...
renderState screenHeight GameState{currentScene = PlayScene, playerPosition = (x, y), enemyPositions = enemies, elapsedTime = elapsed, assets = assets, projectiles = ps} = pictures
    ...
    , pictures (map (\(ex, ey) -> translate ex (ey + 30) $ scale 0.2 0.2 (maybe Blank id (Map.lookup "monster" assets))) enemies) -- Draw enemies
    , pictures [translate (px + 15) (py + 16) $ scale 0.1 0.1 (maybe Blank id (Map.lookup "bullet" assets)) | (px, py) <- ps] -- Draw projectiles
    , translate (-600) (screenHeight / 2 - 260) $ scale 0.2 0.2 $ color white $ text "Move: Left Arrow / Right Arrow" -- Player keybinds
    , translate (-600) (screenHeight / 2 - 300) $ scale 0.2 0.2 $ color white $ text "Shoot: Press Tab" -- Player keybinds
...
renderState _ GameState{currentScene = MenuScene item, assets = assets} = pictures
    ...
    , translate (-150) (-300) $ color white $ scale 0.2 0.2 $ text "Arrows to choose"
    , translate (-150) (-340) $ color white $ scale 0.2 0.2 $ text "Enter to select"
    ]
  where
